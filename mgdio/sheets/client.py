...
token used by Gmail, Calendar, and Drive. Services are cached per resolved
profile slug.
"""
...
def get_service(profile: str | None = None) -> Resource:
    """Return the cached Sheets v4 ``Resource`` for a profile.
...
def reset_service_cache() -> None:
    """Clear all cached Sheets Resources (mainly for tests)."""
    _services.clear()
