...
def reset_caches() -> None:
    """Reset the module-level credential and service caches between tests."""
    gmail_auth.reset_credentials_cache()
...
def tmp_appdata(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        gmail_auth, "GMAIL_CLIENT_SECRET_PATH", tmp_path / "client_secret.json"
    )
    return tmp_path
...
def fake_keyring(monkeypatch):
    """Replace ``keyring`` calls in ``mgdio.gmail.auth`` with an in-memory store."""
    ...
    monkeypatch.setattr(gmail_auth, "keyring", _FakeKeyring)
    return store
